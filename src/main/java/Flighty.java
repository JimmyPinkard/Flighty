...
public class Flighty {
            ...
            if (promptYN("Book this trip?")) {
                ...
                for (int i = 0; i < bookSeats; i++) {
                    ...
                    for (int curr = 0; curr < fl.size(); curr++) {
                        clr();
                        println('\n' + ANSI_WHITE_BG+ANSI_BLACK+"   NOW BOOKING FLIGHT "+(curr+1)+" OF "+fl.size()+"   "+ANSI_RESET+'\n');
                    ...
                    if(i+1 < bookSeats) {
                        println('\n' + ANSI_WHITE_BG+ANSI_BLACK+"   NOW BOOKING NEXT USER "+ANSI_RESET+'\n');
                        clr();
                    }
                }
                return true;
    ...
    public void menuInspectBooking() {
        ...
        println('\n' + ANSI_WHITE_BG + ANSI_BLACK + " YOUR BOOKINGS: " + ANSI_RESET + '\n');
        String[] response = menuLong("Choose a booking to view, or enter '" 
            + options.size() + "' to go back", options);
    ...
    private void menuCancelBooking() {
        ...
        if (response[0].equals(OPTIONS_BACK)) {
            ...
            bookingAgent.unbookListing(userManager.getCurrentUser().getBookingHistory().get(Integer.parseInt(response[1])));
